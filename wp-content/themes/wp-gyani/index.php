        ...
        <div class="max-w-[1280px] mx-auto px-4 md:px-8 relative z-10">
            ...
            <nav aria-label="Breadcrumb" class="mb-6">
                <ol class="flex items-center gap-2 text-sm font-medium text-textmuted dark:text-gray-400">
                    ...
                    <li class="text-charcoal dark:text-gray-200 font-semibold">
            ...
            <p class="font-inter text-sm text-textmuted dark:text-gray-400">
    ...
    <section class="py-16 md:py-20">
        ...
        <div class="max-w-[1280px] mx-auto px-4 md:px-8 mb-12">
            <form action="<?php echo esc_url(home_url('/')); ?>" method="get" class="flex flex-col gap-5">
                ...
                <!-- Filters & Toggles Row -->
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-gray-800 p-3 rounded-2xl border border-bordercolor dark:border-gray-700 shadow-sm">
                    <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                        <span class="text-xs font-bold font-manrope text-textmuted dark:text-gray-400 uppercase tracking-wider ml-2 hidden sm:block">Filter by:</span>
                        ...
                        <div class="relative w-full sm:w-48">
                            ...
                            ?>
                            <select name="cat" onchange="this.form.submit()" class="w-full pl-4 pr-10 py-2.5 rounded-xl border-none focus:ring-2 focus:ring-teal/20 focus:outline-none text-sm text-charcoal dark:text-white bg-offwhite dark:bg-gray-700 appearance-none cursor-pointer font-medium hover:bg-bordercolor/40 dark:hover:bg-gray-600 transition-colors">
                            ...
                            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </div>

                        <!-- Sort Filter -->
                        <div class="relative w-full sm:w-48">
                            <?php $current_sort = isset($_GET['sort']) ? sanitize_text_field($_GET['sort']) : 'newest'; ?>
                            <select name="sort" onchange="this.form.submit()" class="w-full pl-4 pr-10 py-2.5 rounded-xl border-none focus:ring-2 focus:ring-teal/20 focus:outline-none text-sm text-charcoal dark:text-white bg-offwhite dark:bg-gray-700 appearance-none cursor-pointer font-medium hover:bg-bordercolor/40 dark:hover:bg-gray-600 transition-colors">
                    ...
                    <!-- View Toggles -->
                    <div class="hidden md:flex bg-offwhite dark:bg-gray-700 p-1 rounded-xl border border-bordercolor dark:border-gray-600 items-center">
                        <button type="button" id="btn-grid" onclick="setBlogView('grid')" class="p-2 rounded-lg text-teal bg-white dark:bg-gray-800 shadow-sm transition-all" aria-label="Grid View">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <?php if (have_posts()): ?>
                        <div id="articles-grid" class="grid grid-cols-1 md:grid-cols-2 gap-8 transition-all duration-300">
                            <?php while (have_posts()): the_post(); ?>
                                ...
                                ?>
                                <article <?php post_class('bg-white dark:bg-gray-800 rounded-2xl border border-bordercolor dark:border-gray-700 overflow-hidden flex flex-col h-full relative transition-colors hover:border-bordercolor-hover dark:hover:border-gray-500 group/card'); ?>>
                                    <div>
                                        ...
                                        <div class="p-6">
                                            <h2 class="font-manrope font-bold text-xl text-charcoal dark:text-white group-hover/card:text-teal transition-colors leading-snug">
                                                <a href="<?php the_permalink(); ?>" class="before:absolute before:inset-0 before:z-30 hover:underline"><?php the_title(); ?></a>
                                            </h2>
                                            <p class="font-inter text-textmuted dark:text-gray-400 text-sm mt-3 mb-5 line-clamp-2"><?php echo wp_trim_words(get_the_excerpt(), 20); ?></p>
                                            
                                            <div class="meta-footer mt-auto pt-4 border-t border-bordercolor/60 dark:border-gray-700 flex items-center justify-between text-xs text-textmuted dark:text-gray-400 relative z-10">
                                                <span class="font-medium"><?php echo get_the_date(); ?></span>
                                                <span class="font-semibold text-charcoal dark:text-gray-200 bg-offwhite dark:bg-gray-700 px-2 py-0.5 rounded"><?php echo wp_gyani_reading_time(); ?> min read</span>
                                            </div>
                                        </div>
                                    </div>
                                </article>
                        ...
                        <style>
                            ...
                            html.dark .nav-links .page-numbers { border-color: #374151; background: #1f2937; color: #d1d5db; }
                            .nav-links .page-numbers:hover { border-color: #14b8a6; color: #14b8a6; background: #f0fdfa; }
                            html.dark .nav-links .page-numbers:hover { background: #134e4a; border-color: #14b8a6; color: #5eead4; }
                            .nav-links .page-numbers.current { background: #14b8a6; color: #fff; border-color: #14b8a6; }
                            html.dark .nav-links .page-numbers.current { background: #14b8a6; border-color: #14b8a6; color: #fff; }
                            .nav-links .dots { border: none; background: transparent; padding: 0.5rem; pointer-events: none; }
                        </style>
                    <?php else: ?>
                        <div class="text-center py-20 bg-white dark:bg-gray-800 border border-bordercolor dark:border-gray-700 rounded-2xl shadow-sm">
                            <p class="text-textmuted dark:text-gray-400 text-lg mb-6">No articles found.</p>
                ...
                <aside class="lg:col-span-4 space-y-8 lg:sticky lg:top-24 h-max">
                    ...
                    <div class="p-6 rounded-2xl bg-white dark:bg-gray-800 border border-bordercolor dark:border-gray-700 shadow-soft">
                        <h3 class="font-manrope font-extrabold text-lg text-charcoal dark:text-white mb-4 flex items-center gap-2">
                        ...
                        <form action="<?php echo esc_url(home_url('/')); ?>" method="get" class="relative">
                            <input type="search" name="s" value="<?php echo get_search_query(); ?>" placeholder="Search articles..." class="w-full pl-4 pr-12 py-3 rounded-xl border border-bordercolor dark:border-gray-600 focus:outline-none focus:border-teal focus:ring-2 focus:ring-teal/20 text-sm bg-offwhite dark:bg-gray-700 dark:text-white transition-all">
                    ...
                    <div class="p-6 rounded-2xl bg-white dark:bg-gray-800 border border-bordercolor dark:border-gray-700 shadow-soft">
                        ...
                        <ul class="space-y-3 font-medium text-sm text-textmuted dark:text-gray-400">
                            <?php
                            ...
                            <li>
                                <a href="<?php echo esc_url(get_category_link($cat->term_id)); ?>" class="flex items-center justify-between hover:text-teal transition-colors group">
                                    ...
                                    <span class="bg-offwhite dark:bg-gray-700 dark:text-gray-300 px-2 py-0.5 rounded text-xs group-hover:bg-teal-light group-hover:text-teal-dark transition-colors"><?php echo esc_html($cat->count); ?></span>
                                </a>
                            </li>
                            <?php endforeach; ?>
                        </ul>
                    ...
                    <!-- Widget: Newsletter -->
                    <div class="p-6 rounded-2xl bg-gradient-to-br from-teal-light via-white to-offwhite dark:from-gray-800 dark:via-gray-800 dark:to-gray-900 border border-bordercolor dark:border-gray-700 shadow-soft text-center">
                        ...
                        <h3 class="font-manrope font-extrabold text-xl text-charcoal dark:text-white mb-2">Join our Newsletter</h3>
                        <p class="font-inter text-sm text-textmuted dark:text-gray-400 mb-4">Get the latest WordPress tips and tutorials straight to your inbox.</p>
                        <form class="space-y-3">
                            <input type="email" placeholder="Email Address" class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:border-teal text-sm">
                    ...
                    <div class="p-6 rounded-2xl bg-white dark:bg-gray-800 border border-bordercolor dark:border-gray-700 shadow-soft">
                        ...
                        <ul class="space-y-4">
                            ...
                            <li class="flex gap-3 items-start group">
                                <a href="<?php the_permalink(); ?>" class="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-offwhite dark:bg-gray-700">
                                    <?php if (has_post_thumbnail()): ?>
                                ...
                                <div>
                                    <h4 class="font-manrope font-bold text-sm text-charcoal dark:text-gray-200 group-hover:text-teal transition-colors leading-tight line-clamp-2">
                                        <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                    </h4>
                                    <span class="text-xs text-textmuted dark:text-gray-500 mt-1 block"><?php echo get_the_date(); ?></span>
                                </div>
                            </li>
...
<script>
    function setBlogView(view) {
        ...
        if (view === 'list') {
            ...
            btnList.classList.add('bg-white', 'dark:bg-gray-800', 'shadow-sm', 'text-teal');
            btnList.classList.remove('text-gray-400');
            btnGrid.classList.remove('bg-white', 'dark:bg-gray-800', 'shadow-sm', 'text-teal');
            btnGrid.classList.add('text-gray-400');
            localStorage.setItem('blog_view', 'list');
        } else {
            grid.classList.remove('layout-list');
            btnGrid.classList.add('bg-white', 'dark:bg-gray-800', 'shadow-sm', 'text-teal');
            btnGrid.classList.remove('text-gray-400');
            btnList.classList.remove('bg-white', 'dark:bg-gray-800', 'shadow-sm', 'text-teal');
