...
if (post_password_required()) {
    ?>
    <div class="p-6 rounded-2xl bg-offwhite border border-bordercolor text-center">
        <p class="font-inter text-textmuted text-sm">This post is password protected. Enter the password to view comments.</p>
    </div>
    <?php
    return;
}
...
<div id="comments" class="comments-area">
    ...
    <?php if (have_comments()): ?>
        <h2 class="font-manrope font-extrabold text-2xl text-charcoal mb-6">
            <?php
    ...
    <?php if (!comments_open() && get_comments_number() && post_type_supports(get_post_type(), 'comments')): ?>
        <p class="font-inter text-textmuted text-sm mt-8 py-4 border-t border-bordercolor">Comments are closed.</p>
    <?php endif; ?>
    ...
    comment_form(array(
        ...
        'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title font-manrope font-extrabold text-xl text-charcoal mb-1">',
        ...
        'comment_notes_after'  => '',
        'class_form'           => 'comment-form mt-10 pt-8 border-t border-bordercolor space-y-4',
        'class_submit'         => 'submit',
        'label_submit'         => __('Post Comment', 'wp-gyani'),
        'comment_field'        => '
            <p class="comment-form-comment">
                <label for="comment" class="block font-manrope font-bold text-xs uppercase tracking-wider text-charcoal mb-2">Comment <span class="text-teal">*</span></label>
                <textarea id="comment" name="comment" rows="6" required
                    class="w-full px-4 py-3 rounded-xl border border-bordercolor bg-offwhite text-charcoal font-inter text-sm focus:outline-none focus:border-teal focus:bg-white transition-colors resize-y"></textarea>
            </p>',
        'fields' => apply_filters('comment_form_default_fields', array(
            'author' => '
                <p class="comment-form-author">
                    <label for="author" class="block font-manrope font-bold text-xs uppercase tracking-wider text-charcoal mb-2">Name <span class="text-teal">*</span></label>
                    <input id="author" name="author" type="text" value="' . esc_attr(isset($commenter['comment_author']) ? $commenter['comment_author'] : '') . '" required
                        class="w-full px-4 py-2.5 rounded-xl border border-bordercolor bg-offwhite text-charcoal font-inter text-sm focus:outline-none focus:border-teal focus:bg-white transition-colors">
                </p>',
            'email' => '
                <p class="comment-form-email">
                    <label for="email" class="block font-manrope font-bold text-xs uppercase tracking-wider text-charcoal mb-2">Email <span class="text-teal">*</span></label>
            ...
            'url' => '
                <p class="comment-form-url">
                    <label for="url" class="block font-manrope font-bold text-xs uppercase tracking-wider text-charcoal mb-2">Website</label>
        ...
        'logged_in_as'  => sprintf(
            '<p class="logged-in-as font-inter text-sm text-textmuted mb-4">%s <a href="%s" class="text-teal font-semibold hover:text-teal-dark">%s</a>. %s</p>',
            sprintf(esc_html__('Logged in as %s.', 'wp-gyani'), '<span class="text-charcoal font-semibold">' . esc_html(wp_get_current_user()->display_name) . '</span>'),
