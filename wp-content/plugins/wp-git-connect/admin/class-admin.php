...
class Admin {
	...
	public function register_menu() {
		...
		add_submenu_page(
			'wp-git-connect',
			__( 'Status & Sync', 'wp-git-connect' ),
			__( 'Status', 'wp-git-connect' ),
			'manage_options',
			'wp-git-connect-status',
			array( $this, 'render_status' )
		);

		add_submenu_page(
			'wp-git-connect',
			__( 'Commits', 'wp-git-connect' ),
			__( 'Commits', 'wp-git-connect' ),
			'manage_options',
			'wp-git-connect-commits',
			array( $this, 'render_commits' )
		);

		add_submenu_page(
			'wp-git-connect',
			__( 'Branches', 'wp-git-connect' ),
			__( 'Branches', 'wp-git-connect' ),
			'manage_options',
			'wp-git-connect-branches',
			array( $this, 'render_branches' )
		);

		add_submenu_page(
			'wp-git-connect',
			__( 'Activity Logs', 'wp-git-connect' ),
			__( 'Activity Logs', 'wp-git-connect' ),
			'manage_options',
			'wp-git-connect-logs',
		...
		add_submenu_page(
			'wp-git-connect',
			__( 'Settings', 'wp-git-connect' ),
			__( 'Settings', 'wp-git-connect' ),
			'manage_options',
			'wp-git-connect-settings',
	...
	public function enqueue_scripts( $hook ) {
		if ( strpos( $hook, 'wp-git-connect' ) === false ) {
			return;
		}
	...
	/**
	 * Handle form submissions like settings save.
	 */
	public function handle_form_submissions() {
		if ( isset( $_POST['wp_git_connect_save_settings'] ) && isset( $_POST['wp_git_connect_nonce'] ) ) {
			if ( wp_verify_nonce( $_POST['wp_git_connect_nonce'], 'wp_git_connect_settings_action' ) && current_user_can( 'manage_options' ) ) {
				...
				$settings['repo_path'] = sanitize_text_field( $_POST['repo_path'] );
				$settings['remote_url'] = sanitize_text_field( $_POST['remote_url'] );
				$settings['default_branch'] = sanitize_text_field( $_POST['default_branch'] );
				$settings['auth_method'] = sanitize_text_field( $_POST['auth_method'] );
				$settings['username'] = sanitize_text_field( $_POST['username'] );
				
				if ( ! empty( $_POST['pat_token'] ) ) {
					$settings['pat_token'] = sanitize_text_field( $_POST['pat_token'] );
				}
				...
				if ( isset( $_POST['mysqldump_path'] ) ) {
					$settings['mysqldump_path'] = sanitize_text_field( $_POST['mysqldump_path'] );
				}
				if ( isset( $_POST['mysql_path'] ) ) {
					$settings['mysql_path'] = sanitize_text_field( $_POST['mysql_path'] );
				}
	...
	public function render_dashboard() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/dashboard.php';
	}
	...
	public function render_status() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/status.php';
	}

	/**
	 * Render Commits View.
	 */
	public function render_commits() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/commits.php';
	}
	...
	public function render_branches() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/branches.php';
	}
	...
	public function render_logs() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/logs.php';
	}
	...
	public function render_settings() {
		require_once WP_GIT_CONNECT_PATH . 'admin/views/settings.php';
	}
}
