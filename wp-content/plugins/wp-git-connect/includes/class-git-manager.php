...
class Git_Manager {
	...
	public static function fetch() {
		...
		if ( ! $remote_url ) {
			return array( 'status' => false, 'message' => 'Remote URL not configured.' );
		}
	...
	public static function commit_and_push( $files, $message, $branch = 'main' ) {
		...
		$push_result = Git_Command::execute( 'push', array( 'origin', $branch ) );
		if ( $push_result['status'] ) {
			Git_Logger::log( 'PUSH', 'Success', "Pushed to origin/{$branch}.", $branch, $hash );
			return array( 'status' => true, 'message' => 'Commit and Push successful.' );
		}

		Git_Logger::log( 'PUSH', 'Failed', $push_result['error'], $branch, $hash );
